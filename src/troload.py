...
import std_logging as logging
from std_main import StandardApp
import std_report as rpt
...
def get_next_file(this_app, stage_dir):
    ...
    this_app.logger.debug(f"end   get_next_file - retuns {file_name=}")
    return file_name
...
def process_file(file_name, log, rpt, db_conn):
    ...
    tran_tab = TransactionsTable(db_conn)
...
def main():
    ...
    while file_name:
        ...
        this_app.logger.info("No new files were found")
        this_app.report.write("No new files were found\n")
        rc = 1
