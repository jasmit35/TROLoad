...
class TransactionsProcessor:
    ...
    def process_one_file(self):
        ...
        #  Now we need to read the transactions from the Excel file.
        ...
        cols = ["Date", "Account"]
        df.loc[:, cols] = df.loc[:, cols].ffill()  # Fill in NaN values with the previous value
        print(df.head(10))
        print(df.tail(10))
        return 0

        #   def read_value_from_excel0(filename, column="C", row=3):
        ...
        # date_string = pd.read_excel(
        #     self._file_path, engine="openpyxl", skiprows=2, usecols="A", nrows=1, header=None, names=["Value"]
        #  ).iloc[0]["Value"]
        #         excel_data = pd.read_excel(self._file_path)
        #         date_string = excel_data.at[3, 1]
        # print(date_string)

        #          excel_data.close()
        #  df = df.drop(index=[1, 2, 3], axis=0)
        #  df = df.drop(df.columns[[0]], axis=1)
        #  df.to_excel(self._file_path, engine="openpyxl")
        ...
        #     "Amount",
        # ]
        # data = pd.read_csv(self._file_path, encoding="ISO-8859-1", delimiter=",", skiprows=3, names=col_names)
        #  data = pd.read_csv(self._file_path, encoding="ISO-8859-1", delimiter=",")
        # . print(data.head(10))
        #  print(data.tail())
        # start_date, end_date = self.extract_date_range()
